...
import Plutarch.Api.V2 (PScriptContext, PScriptPurpose (..), PStakeValidator, PStakingCredential (..), PValidator)
...
main = phoistAcyclic $
    plam $ \redeemer ctx -> unTermCont $ do
        ptraceC "main"
        ctxF <- pletFieldsC @'["txInfo", "purpose"] ctx
        _txInfoF <- pletFieldsC @'["wdrl"] ctxF.txInfo
        return $
            pmatch ctxF.purpose $ \case
                PMinting ((pfield @"_0" #) -> _policy) -> do
                    (popaque $ pconstant ())
